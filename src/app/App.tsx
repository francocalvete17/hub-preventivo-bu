...
import {
  ...
  CheckCircle, BarChart2, Package, Clock, TrendingUp, X,
} from "lucide-react";
    ...
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-2 items-center pointer-events-none">
      {toasts.map(t => (
        <div key={t.id} className="flex items-center gap-3 px-4 py-2.5 rounded-xl pointer-events-auto"
          style={{ background: "#0f172a", border: "1px solid rgba(255,255,255,0.1)", color: "#f1f5f9", fontFamily: "'DM Mono', monospace", fontSize: "12px", letterSpacing: "0.01em", boxShadow: "0 8px 32px rgba(0,0,0,0.6)", minWidth: "260px" }}>
          <span style={{ color: t.type === "success" ? "#4ade80" : t.type === "error" ? "#f87171" : "#60a5fa", fontSize: "14px" }}>
            {t.type === "success" ? "✓" : t.type === "error" ? "✕" : "ℹ"}
          </span>
          <span className="flex-1">{t.message}</span>
          <button onClick={() => onRemove(t.id)} className="hover:opacity-70 transition-opacity" style={{ color: "#475569" }}><X size={12} /></button>
        </div>
      ))}
    </div>
  );
